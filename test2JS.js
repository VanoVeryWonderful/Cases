//let oneImage = document.getElementById("main_Image");
...
//document.getElementById("pushMe").onсlick = function circle() {
//		oneImage.src = images[1];
//}
...
document.getElementById("main_Block_for_Img").onwheel = function circle(event) {
	...
	if (currentImageIndex === images.length - 1 || event.deltaY < 0 ) {
		currentImageIndex += 1;
		} 
		else if(currentImageIndex === images.length - 1 || event.deltaY > 0) {
		currentImageIndex -= 1; 
	}
	imageElement.src = images[currentImageIndex]; 
	return false;
